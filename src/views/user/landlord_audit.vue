          ...
          <el-form :inline="true" :model="user" class="demo-form" size="mini" :rules="rules" ref="user">
            <el-divider content-position="left">基本信息</el-divider>
             <el-form-item label="姓名：" prop="name">
              <el-input v-if="user.user" v-model="user.user.name" disabled></el-input>
            </el-form-item>
              <el-form-item label="身份证号码：" prop="id_number">
              <el-input v-model="user.id_number" disabled></el-input>
            </el-form-item>
             <el-form-item label="身份证正面照片：" prop="id_card_picture_f_path"  v-if="user.user">
               <!-- <div class="demo-image">
               <div class="block">
               <el-image src="@/assets/imgs/440184199812220312f.jpg"></el-image>
              </div>
              </div> -->
               <!-- <el-input v-model="user.id_card_picture_f_path" disabled></el-input> -->
              <img style="width: 100px; height: 100px" :src="require('../../assets/imgs/'+user.id_card_picture_f_name)" alt="" v-if="user.user">
              <!-- <img style="width: 100px; height: 100px" src="@/assets/imgs/440184199812220312f.jpg" alt=""> -->
              <!-- <el-image style="width: 100px; height: 100px" :src="user.id_card_picture_f_path" fit="cover"></el-image> -->
            </el-form-item>
            <el-form-item label="身份证反面照片：" prop="id_card_picture_r_path" v-if="user.user">
              <!-- <el-image
              style="width: 100px; height: 100px"
              :src="user.id_card_picture_r_path"
              ></el-image> -->

              <!-- <img style="width: 100px; height: 100px" :src="user.id_card_picture_r_path" alt="" v-if="user.user">
              <img style="width: 100px; height: 100px" src="@/assets/imgs/440184199812220312r.jpg" alt=""> -->
              <img style="width: 100px; height: 100px" :src="require('../../assets/imgs/'+user.id_card_picture_r_name)" alt="" v-if="user.user">
            </el-form-item>
            <br>
...
<!-- 主界面 -->
      ...
      <el-main>
        <el-table :data="userList" border style="width: 100%;" v-loading="loading">
          <el-table-column :resizable="false" prop="user_id" label="用户id" min-width="25%" ></el-table-column>
          <el-table-column :resizable="false" prop="user.name"  label="姓名" min-width="40%"></el-table-column>
          <el-table-column :resizable="false" prop="id_number"  label="身份证号码"  min-width="65%"></el-table-column>
          <el-table-column :resizable="false" prop="alipay_name"  label="支付宝账户名" min-width="65%"></el-table-column>
          <el-table-column :resizable="false" prop="alipay_account"  label="支付宝账号"  min-width="65%"></el-table-column>
          ...
          <el-table-column :resizable="false" fixed="right"  label="操作">
            <template slot-scope="scope">
              <el-button  v-if="scope.row.status !== '审批通过' " @click="edit(scope.row.user_id)" type="primary" size="small">编辑</el-button>
  ...
  methods: {
    ...
    pageInation (users) {
      ...
      if (users === null || users === undefined) {
        this.$http.get('findLandlordInformationList').then(res => {
    ...
    currentChange (current) {
      ...
      // 姓名模糊查询的分页：
      if (this.xuser.name !== '') {
        this.$http.get('findLandlordInformationList?name=' + this.xuser.name).then(res => {
          // console.log(res.data)
    ...
    edit (id) {
      ...
      this.$http.get(`findLandlordInfoDetail?user_id=${id}`).then(res => {
    ...
    ConfirmUpdateUser (id, formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          // })
          this.$http({
            url: 'adminUpdateLandlordInfomation',
            method: 'post',
            params: {
              landlordInformationDTO: this.user
            },
            headers: {
